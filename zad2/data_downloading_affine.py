...
def main_afinic():
    ...
    np.savez("data/example_submission_affine.npz", ids=np.array(all_indices), representations=data_A)


def main_afinic_with_voting():
...
def main_afinic_with_validation_set():
    all_indices = get_all_indices()
    ...
    while new_idx < 20000:
        ...
        test_values = [[] for _ in range(NUMBER_OF_TRIES)]
        validation_values = [[] for _ in range(NUMBER_OF_TRIES)]
        test_data_B = None
        ...
        for i in range(NUMBER_OF_TRIES):
            ...
            while True:
                try:
                    ...
                    # get INDEX_STEP values from endpoint B
                    test_data_B = download_data(all_indices[new_idx:new_idx + INDEX_STEP], EndpointType.DEFENDER.value,
                                                EndpointEncoding.AFFINE.value)
                    break
            ...
            # split the data from matrix A and B into train and validation variables
            train_variable_A = train_data_A[:int(TRAIN_VAL_SPLIT * len(train_data_A))]
            train_variable_B = train_data_B[:int(TRAIN_VAL_SPLIT * len(train_data_B))]
            val_variable_A = train_data_A[int(TRAIN_VAL_SPLIT * len(train_data_A)):]
            val_variable_B = train_data_B[int(TRAIN_VAL_SPLIT * len(train_data_B)):]
            # calculate X from BX = A
            X = np.linalg.lstsq(train_variable_B, train_variable_A, rcond=None)[0]

            validation_error = np.linalg.norm(val_variable_B @ X - val_variable_A)
            print(f"[INFO] Error: [{np.linalg.norm(train_data_B @ X - train_data_A):0.8f}]")
            print(f"[INFO] Validation error: [{validation_error:0.8f}]")
            ...
            test_values[i] = A_test_new
            validation_values[i] = validation_error
        ...
        # get the value with the lowest validation error
        A_test_new = test_values[np.argmin(validation_values)]
        # concatenate new data to the old one
        data_A = np.concatenate([data_A, A_test_new], axis=0)
...
def get_random_solution():
    all_indices = get_all_indices()
    # data_A = (np.random.normal(loc=0., scale=1.7, size=(20000, 384)))
...
if __name__ == "__main__":
    # get_random_solution()
    # main_afinic()
    # main_afinic_with_voting()
    main_afinic_with_validation_set()
